...
def step_generate_search_terms():
    ...
    if 'search_terms' not in st.session_state or not st.session_state.search_terms:
        try:
            with st.spinner("Generating search terms for all concepts..."):
                # Get the list of concept texts
                concepts_list = [concept['text'] for concept in st.session_state.concepts]
                # Call the AI function to generate search terms for all concepts
                search_terms_dict = ai_utils.generate_search_terms_all(concepts_list)
                # Store in session state
                st.session_state.search_terms = search_terms_dict
        except Exception as e:
